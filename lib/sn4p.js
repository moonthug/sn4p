...
var USER_AGENT          = 'Snapchat/9.10.0.0 (HTC One; Android 4.4.2#302626.7#19; gzip)',
...
Sn4p.prototype.login = function(cb) {
    ...
    async.waterfall([
        ...
        function(deviceToken, authToken, next) {
            self._getAttestation(self.snapchatCredentials.password, timestamp, function(err, data) {
 ...
 * @private
 */
Sn4p.prototype._getAttestation = function(password, timestamp, cb) {
    var hashString = this.snapchatCredentials.username + '|' + password + '|' + timestamp + '|/loq/login';
